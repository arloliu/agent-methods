...
| `tag-exists-remote` | The request names `alpha/v0.2.0`, which exists on `origin` at the baseline but not locally | Query the remote view before preparing, report `not-ready` with no version-reference commit, and leave the remote tag unchanged; deleting or re-pointing it is never offered |
