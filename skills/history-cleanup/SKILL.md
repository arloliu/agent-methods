...
description: >
  ...
  For these requests, read all of SKILL.md before issuing Git commands;
  follow its conditional references when needed.
...
Preserve exact tree identity: `original HEAD^{tree} == rewritten HEAD^{tree}`.
...
Use `git --no-optional-locks rev-list --reverse --topo-order <merge-base>..HEAD`.
...
Resulting group order: <group IDs in resulting order, including a single group>
...
- Identify each pending repository check, including behavioral checks, by its command or unavailable tool.
  Mark it not-run or incomplete with the reason; inspection does not establish that a check passed.
...
Checks pending: <each required check with its command or unavailable tool; not-run or incomplete; reason, or none>
After approval:
...
Read [Rewrite and verify](references/rewrite.md) in full before any approved checks or Git mutations.
For a separate publication request, read its publishing section before acting.
